...
public class GmsCore {
    ...
    private static InstallResult installPackages(Set<String> list, int userId) {
        ChiyuanVACore vaCore = ChiyuanVACore.get();
        for (String packageName : list) {
            if (vaCore.isInstalled(packageName, userId)) {
                continue;
            }
            try {
                ChiyuanVACore.getContext().getPackageManager().getApplicationInfo(packageName, 0);
            } catch (PackageManager.NameNotFoundException e) {
                
                continue;
            }
            InstallResult installResult = vaCore.installPackageAsUser(packageName, userId);
    ...
    private static void uninstallPackages(Set<String> list, int userId) {
        ChiyuanVACore vaCore = ChiyuanVACore.get();
        for (String packageName : list) {
            vaCore.uninstallPackageAsUser(packageName, userId);
        }
    }
    ...
    public static boolean isSupportGms() {
        try {
            ChiyuanVACore.getPackageManager().getPackageInfo(GMS_PKG, 0);
            return true;
    ...
    public static boolean isInstalledGoogleService(int userId) {
        return ChiyuanVACore.get().isInstalled(GMS_PKG, userId);
    }
}