package com.chiyuan.va.fake.frameworks;
...
import com.chiyuan.va.core.system.pm.IBPackageManagerService;
import com.chiyuan.va.entity.pm.InstallOption;
import com.chiyuan.va.entity.pm.InstallResult;
import com.chiyuan.va.entity.pm.InstalledPackage;
import com.chiyuan.va.utils.TransactionThrottler;
...
public class BPackageManager extends BlackManager<IBPackageManagerService> {
    ...
    public Intent getLaunchIntentForPackage(String packageName, int userId) {
        ...
        List<ResolveInfo> ris = queryIntentActivities(intentToResolve,
                0,
                intentToResolve.resolveTypeIfNeeded(ChiyuanVACore.getContext().getContentResolver()),
                userId);
    ...
    private Intent createFallbackLaunchIntent(String packageName) {
        try {
            ...
            Intent intent = ChiyuanVACore.getContext().getPackageManager().getLaunchIntentForPackage(packageName);
            if (intent != null) {
                intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                return intent;
            }
        ...
        try {
            ...
            if (file != null && !file.isEmpty()) {
                try {
                    ...
                    PackageInfo packageInfo = ChiyuanVACore.getPackageManager().getPackageArchiveInfo(file, 0);
                    if (packageInfo != null) {
                        String packageName = packageInfo.packageName;
                        String hostPackageName = ChiyuanVACore.getHostPkg();
                        if (packageName.equals(hostPackageName)) {
                            Log.w(TAG, "Attempt to install ChiyuanVA app detected and blocked: " + packageName);
    ...
    private boolean isInstalledFallback(String packageName) {
        try {
            ...
            ChiyuanVACore.getContext().getPackageManager().getPackageInfo(packageName, 0);
            return true;
        } catch (Exception e) {
            ...
            if (packageName != null && (packageName.equals("com.media.bestrecorder.audiorecorder") || 
                                       packageName.startsWith("com.chiyuan.va"))) {
