...
import com.chiyuan.va.entity.pm.InstallResult;

import com.chiyuan.va.fake.delegate.ContentProviderDelegate;
import com.chiyuan.va.fake.frameworks.BActivityManager;
import com.chiyuan.va.fake.frameworks.BJobManager;
import com.chiyuan.va.fake.frameworks.BPackageManager;
import com.chiyuan.va.fake.frameworks.BStorageManager;
import com.chiyuan.va.fake.frameworks.BUserManager;
...
import com.chiyuan.va.utils.ShellUtils;
import com.chiyuan.va.utils.Slog;
import com.chiyuan.va.utils.SimpleCrashFix;
import com.chiyuan.va.utils.compat.BuildCompat;
import com.chiyuan.va.utils.compat.BundleCompat;

import com.chiyuan.va.utils.provider.ProviderCall;
import com.chiyuan.va.utils.StackTraceFilter;
import com.chiyuan.va.utils.SocialMediaAppCrashPrevention;
import com.chiyuan.va.utils.DexCrashPrevention;
import com.chiyuan.va.utils.NativeCrashPrevention;
import com.chiyuan.va.utils.CrashMonitor;
import com.chiyuan.va.utils.StoragePermissionHelper;
import com.chiyuan.va.utils.LogSender;
...
public class ChiyuanVACore extends ClientConfiguration {
    ...
    public static ChiyuanVACore get() {
        return sChiyuanVACore;
    }
    ...
    public void setCurrentAppUid(int uid, String packageName) {
        ...
        if (uid != mHostUid && uid > Process.FIRST_APPLICATION_UID && uid < Process.LAST_APPLICATION_UID) {
            ...
            Slog.d("ChiyuanVACore", "Detected sandboxed environment for " + packageName + " with UID: " + uid);
        }
    }
    ...
    public int resolveUidForOperation(int originalUid, String operation) {
        try {
            ...
            if (mIsSandboxedEnvironment && mCurrentAppUid > 0) {
                Slog.d("ChiyuanVACore", "Resolving UID for " + operation + ": " + originalUid + " -> " + mCurrentAppUid);
            ...
            Slog.e("ChiyuanVACore", "Error resolving UID for " + operation + ": " + e.getMessage());
            return originalUid;
        }
    }
    ...
    public boolean areServicesAvailable() {
        ...
        try {
            ...
            if (isMainProcess() && !isVAProcessRunning()) {
                Slog.w(TAG, "VA process not running, starting it and using fallback services...");
                startBlackProcess();
    ...
    private IBinder getServiceInternal(String name) {
        ...
        if (isMainProcess() && !isVAProcessRunning()) {
            Slog.w(TAG, "Main process trying to access service " + name + " but VA process not running, starting it...");
            startBlackProcess();
    ...
    private boolean isVAProcessRunning() {
        try {
            ...
            try {
                ...
                if (result != null) {
                    Slog.d(TAG, "VA process is running - SystemCallProvider accessible");
                    return true;
                }
            } catch (Exception e) {
            ...
            try {
                ...
                if (authority != null && !authority.isEmpty()) {
                    ...
                    if (providerInfo != null) {
                        Slog.d(TAG, "Provider exists but call failed - VA process may be starting");
                        return false; 
                    }
                }
            } catch (Exception e) {
                ...
                if (am != null) {
                    for (android.app.ActivityManager.RunningServiceInfo service : am.getRunningServices(Integer.MAX_VALUE)) {
                        if (service.service.getClassName().contains("DaemonService")) {
                            Slog.d(TAG, "DaemonService is running - VA process active");
                            return true;
                        }
                    }
                }
            ...
            Slog.d(TAG, "VA process is not running");
            return false;
            
        } catch (Exception e) {
            Slog.w(TAG, "Error checking VA process status: " + e.getMessage());
            return false;
        }
    }
    ...
    private void startBlackProcess() {
        try {
            Slog.d(TAG, "Starting VA process...");
            ...
            Slog.d(TAG, "Started DaemonService to initialize VA process");
        } catch (Exception e) {
            Slog.e(TAG, "Failed to start VA process", e);
            ...
            handler.postDelayed(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        Bundle result = ProviderCall.callSafely(ProxyManifest.getBindProvider(), "VM", null, testBundle);
                        if (result != null) {
                            Slog.d(TAG, "VA process started successfully, SystemCallProvider is accessible");
                        } else {
                            Slog.w(TAG, "VA process started but SystemCallProvider is not accessible yet");
                        }
                    } catch (Exception e) {
    ...
    public void ensureBlackProcessInitialized() {
        if (isMainProcess() && !isVAProcessRunning()) {
            Slog.w(TAG, "Ensuring VA process is initialized...");
            startBlackProcess();
            
            
            int maxRetries = 5;
            int retryCount = 0;
            while (retryCount < maxRetries && !isVAProcessRunning()) {
                try {
                    Thread.sleep(500);
                    retryCount++;
                } catch (InterruptedException e) {
            ...
            if (isVAProcessRunning()) {
                Slog.d(TAG, "VA process initialized successfully");
            } else {
                Slog.w(TAG, "VA process failed to initialize, using fallback services");
            }
        }
    }
    ...
    public void doAttachBaseContext(Context context,
        ...
        if (processName.equals(ChiyuanVACore.getHostPkg())) {
        ...
        if (ChiyuanVACore.get().isBlackProcess()) {
            BEnvironment.load();
    ...
    public void doCreate() {
        ...
        try {
            ...
            Slog.d(TAG, "ChiyuanVA initialization completed in " + totalTime + "ms");
            
        } catch (Exception e) {
            long totalTime = System.currentTimeMillis() - startTime;
            Slog.e(TAG, "ChiyuanVA initialization failed after " + totalTime + "ms", e);
    ...
    public InstallResult installPackageAsUser(String packageName, int userId) {
        try {
            ...
            if (packageName.equals(getHostPkg())) {
                return new InstallResult().installError("Cannot clone ChiyuanVA app from within ChiyuanVA. This would create infinite recursion and is not allowed for security reasons.");
            }
    ...
    private void initNotificationManager() {
        ...
        String CHANNEL_ONE_ID = ChiyuanVACore.getContext().getPackageName() + ".chiyuanva_core";
        String CHANNEL_ONE_NAME = "chiyuanva_core";
        if (BuildCompat.isOreo()) {
    ...
    public void closeCodeInit(){
        try {
            Class entry = Class.forName("com.chiyuan.va.closecode.Entry");
    ...
    public void onBeforeMainLaunchApk(String packageName,int userid) {
        for (AppLifecycleCallback appLifecycleCallback : ChiyuanVACore.get().getAppLifecycleCallbacks()) {
    ...
    public static com.chiyuan.va.entity.AppConfig getAppConfig() {
        try {
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        Intent vpnIntent = new Intent(getContext(), com.chiyuan.va.proxy.ProxyVpnService.class);
    ...
    public static boolean isRunningApplication(String packageName, int userId) {
        ...
        try {
            ...
            ServiceManager.get();
            com.chiyuan.va.core.system.am.ActivityStack stack =
                (com.chiyuan.va.core.system.am.ActivityStack) ServiceManager.getService(ServiceManager.ACTIVITY_MANAGER);
            if (stack == null) return false;
            java.util.Map<Integer, com.chiyuan.va.core.system.am.TaskRecord> tasks =
                    com.chiyuan.va.utils.Reflector.with(stack).field("mTasks").get();
            if (tasks == null) return false;
            for (com.chiyuan.va.core.system.am.TaskRecord task : tasks.values()) {
                if (task.userId == userId && task.taskAffinity != null && task.taskAffinity.contains(packageName)) {
                    
                    for (com.chiyuan.va.core.system.am.ActivityRecord activity : task.activities) {
    ...
    public boolean isChiyuanVAApp(File apkFile) {
        try {
            ...
            Slog.w(TAG, "Error checking if APK is ChiyuanVA app: " + e.getMessage());
        }
        return false;
    }
    
    
    public boolean isChiyuanVAApp(String packageName) {
            ...
            try {
                ...
                if (error != null) {
                    ...
                    if (getContext() != null) {
                        ...
                        if (nm != null) {
                            String channelId = getContext().getPackageName() + ".chiyuanva_core";
                            ...
                            builder.setSmallIcon(android.R.drawable.stat_notify_error)
                                   .setContentTitle("ChiyuanVA Log Upload Failed")
                    ...
                    if (getContext() != null) {
                        ...
                        if (nm != null) {
                            ...
                            builder.setSmallIcon(android.R.drawable.stat_sys_upload_done)
                                   .setContentTitle("ChiyuanVA Log Upload")
